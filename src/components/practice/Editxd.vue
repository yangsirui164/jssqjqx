<template>
  <div>
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>想定新增/编辑</el-breadcrumb-item>
    ...
    <el-card>
      ...
      >
        <div class="bao">
          ...
          <el-form-item label="训练计划名称:" prop="name" label-width="115px" class="formitem">
            <el-input v-model="addform.name" size="mini" class="smal"></el-input>
          </el-form-item>
          <el-form-item prop="subjectId"  class="formitem">
            <div class="mofang">
            <span style="color:#F56C6C;margin-right: 4px">*</span><span>科目:</span>
            </div>
            <select v-model="addform.subjectId" class="sles" @change="changesub">
          ...
          >
            <div class="mofang">
            <span style="color:#F56C6C;margin-right: 4px">*</span><span>授课班级:</span>
            </div>
            <select v-model="addform.gradeId" class="sles" @change="changesub1">
              <option disabled value="">请选择</option>
              <option :value="item.id" v-for="(item,index) in gradearr">
                {{item.name}}
              </option>
            </select>
          </el-form-item>
          ...
          >
            <div class="mofang">
              <span style="color:#F56C6C;margin-right: 4px">*</span><span>训练类型:</span>
            </div>
            <select v-model="addform.type" class="sles" @change="changetype">
              <option disabled value="">请选择</option>
          ...
          <el-form-item prop="thinktime">
            <div class="mofang">
              <span style="color:#F56C6C;margin-right: 4px">*</span><span>训练计划时间:</span>
            </div>
          <el-date-picker
            size="mini"
            v-model="addform.thinkTime1"
            type="date"
            placeholder="请选择日期"
         ...
         <span>--</span>
          ...
          >
            ...
            <span>{{hourlen}}小时{{minlen}}分钟</span>
          </el-form-item>

        <el-form-item  prop="teacherId"  class="mgtop">
          <div class="mofang">
            <span style="color:#F56C6C;margin-right: 4px">*</span><span>授课教师:</span>
          </div>
          <select v-model="addform.teacherId" class="sles" @change="changesub2">
            <option disabled value="">请选择</option>
            <option :value="item.id" v-for="(item,index) in teacherarr">
              {{item.realName}}
            </option>
          </select>
        </el-form-item>
...
<!--拖拽-->
        <el-form-item label="顺序拖拽:" label-width="115px" width="100px" class="mgtop">
     <div class="copy">
     <el-tree
       :data="eltreedata"
       :props="treeProps"
     ...
     >
        ...
        <el-form-item label="备注:" prop="remark" label-width="115px" class="mgtop"
        >
          <el-input
            class="contentcla"
            type="textarea"
...
<!--           <div>{{scope.row}}</div>-->
            <el-cascader
              ...
              placeholder="请选择"
              v-model="scope.row.studentId"
              :options="getoptions(scope.row,scope.$index)"
              :props="peopleProps"
              class="singl"
              @change="rychange"
            ></el-cascader>
        ...
        <el-table-column width="160px"
                         ...
                         prop="caozuo">
          <template slot-scope="scope">
<!--            <span @click="edittests(scope.row,scope.row.id)">编辑</span>-->
...
<!--            <span>|</span>-->
      ...
      <el-button @click="addRow()" class="addbutton">新增</el-button>

    </el-card>

    <el-card class="botmcard">
      <el-form
        :model="addform" :rules="addFormRules1" ref="addFormRef1"
        label-width="70px"
        class="addformclas">
      <el-form-item label="上传文件:" prop="upload" label-width="115px" v-if="wenben">
        ...
        <el-upload
          :file-list="filearr"
          action="http://192.168.1.15:8002/api/Home/uploadsource"
      ...
      <el-row type="flex" justify="end">
        <el-button class="savestyle"
                   @click="quxiao">取消</el-button>
...
<!--    <el-dialog-->
<!--      title="新增课程"-->
<!--      :visible.sync="dxrudialogvisible"-->
<!--      width="30%"-->
<!--      @close="dxrudialogclosed"-->
<!--    >-->
<!--      <el-tree-->
<!--        :data="tree_data"-->
<!--        :props="defaultProps"-->
...
<!--    <el-button @click="dxrudialogclosed">取 消</el-button>-->
<!--    <el-button type="primary" @click="updatedxrw">确 定</el-button>-->
<!--  </span>-->
<!--    </el-dialog>-->
...
<script>
    ...
    data(){
      ...
      var checkJuese=(rule,value,callback)=>{
        ...
        if(this.singlevalue.length==0){
          return callback(new Error('请选择单项任务'));
        }
      ...
      return{
        ...
        addFormRules:{
          ...
          singelid:[
            {  required: true,validator:checkJuese,trigger:'change' }
          ],
          xlarea:[
            {  required: true,validator:checkArea,trigger:'change' }
          ]
          // subjectId:[
          //   {  required: true,validator:checkSubject,trigger:'blur'}
          // ]
  ...
  //
  //      that.$refs.cacader.getCheckedNodes().forEach((item,index)=>{
  //         if(item.label==e.toElement.innerText){
  //          newtag=document.createElement("span")
  //          newtag.innerHTML=item.label
  //
  //         }
  //      })
  //
 ...
 //        }
 //      },
    //   "addform.planType":function(val){
    //     console.log('这里')
    //     console.log(val)
    //     if(!val){
    // this.showshunxu=false
    //     }
    //     if(val==1){
    //       //训练
    //       this.showshunxu=false
    //     }
    //     if(val==0){
    //       this.showshunxu=true
    //     }
    //   },
     "addform.thinkTime2":function(val){

           if(val.length>0){
             this.start0time=val[0]
             this.end0time=val[1]
             this.notime=false
           }else{
             this.notime=true
      ...
      rychange(val){
       ...
       this.userlist.forEach((item,index)=>{
        ...
        item.studentId.forEach((item1,index1)=>{
          item.studentId[index1]=item1[0]
        })
         }
      this.choosearr.push(item.studentId)
       })
  ...
  console.log(this.choosearr)
      ...
      getClassList(){
        this.$http.get('api/Grade/getAllGrade').then(res=>{
          ...
          // if(this.order==''){
          //   this.addform.gradeId=res.data[0].id
          //   this.getclassStus()
          // }
      ...
      getAllteacher(){
        this.$http.get(`api/Teacher/getTeacherBySubject/${this.addform.subjectId}`).then(res=>{
          ...
          // if(this.order==''){
          //   this.addform.teacherId=res.data[0].id
          // }
     ...
     console.log(node)
      ...
      getoptions(scoperow,index){
       console.log('此处赋值options')
      console.log(this.choosearr)
        // this.nowoptions=
      ...
      rwchge(val){
        ...
        //树形结构方案
        // this.checkList1.forEach((item,index)=>{
        //
        //  if(item.length==1){
        //
        //    this.eltreedata.push({
        //      label:item[0],
        //      value:item[0]
        //    })
      ...
      test(scoperow){
       // console.log('..............')
       //  console.log(scoperow.thinkstart)
        if(scoperow.thinkstart&&scoperow.thinkend){
          var firsttidx=scoperow.thinkstart.indexOf(":")
          var firsttime0=Number(scoperow.thinkstart.substring(0,firsttidx))
          // console.log("时间1的前面"+firsttime0)
          var endtime0=Number(scoperow.thinkstart.substr(firsttidx+1))
          // console.log('时间1的后面'+endtime0)
          var secondindex=scoperow.thinkend.indexOf(":")
          var firsttime1=Number(scoperow.thinkend.substring(0,secondindex))
          var endtime1=Number(scoperow.thinkend.substr(secondindex+1))
       // console.log('第一个时间'+firsttime0+'--'+firsttime1+'第二个时间'+endtime0+'--'+endtime1)
          if(firsttime0>firsttime1){
            // console.log('走1')
            scoperow.timetest=false
            return true
          }else if(firsttime0==firsttime1){
              if(endtime0>endtime1||endtime0==endtime1){
                // console.log('走2')
                scoperow.timetest=false
                return true
            }else{
                // console.log('走3')
                scoperow.timetest=true
                return false
              }
