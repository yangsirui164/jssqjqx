...
<!--          </div>-->
        ...
        <div id="wrap2">
          ...
          <div class="smw">
            <div class="kuang"  @click="$router.push('/firststudy')">
      ...
      <div class="container">
       <div v-if="iskaoshi=='0'" class="haskaoshi">
        ...
        <div v-if="iskaoshi=='1'" class="haskaoshi">
          <div v-if="ingkaoshi=='11'" class="notask">
            您当前暂无考试信息！
          </div>
          <div v-if="ingkaoshi=='12'">
            <h2 class="title1">
              {{kaoshiform.title}}
            </h2>
            <div class="crosstime">
              <span>考试时长:</span>
              <span>{{kaoshiform.time}}</span>
            </div>
            <div class="crosstime">
              <span>考试开始时间:</span>
              <span>{{kaoshiform.startTime}}</span>

            </div>
            <div class="crosstime crosstime1">
              <span>考试倒计时:</span>
              <span>{{this.m}}分</span>
              <span>{{this.s}}秒</span>
            </div>
            <el-button class="elbtn" :disabled="isprepare"
...
<script>
  export default {
    ...
    methods:{
      ...
      gettaskinfo(){
       this.$http.get(`${this.base}/api/test/getNewStudentTest/${this.loginid}`).then(res=>{
         console.log('获取考试信息')
         ...
         if(res.data.status==400){
           ...
           clearInterval(this.timer1)
           this.kaoshiform=res.data.data
           this.kaoshiid=res.data.data.id
           this.ingkaoshi='12'
      ...
      this.isprepare=true
       clearInterval(this.timer1)
       document.querySelector(".elbtn").innerHTML='等待进入考试'
      },
      hrefgonggao(id){
        console.log('id在这里')
        console.log(id)
