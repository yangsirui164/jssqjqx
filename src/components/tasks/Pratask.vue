      ...
      <div class="nr_left">
        <div class="test">
          <div class="test_title">
            ...
            <div class="fg">
                            <input type="button" name="" value="退出" class="jiaojuan1"
                                   @click="tuichu()">
            </div>
            <div class="fg">
<!--              <input type="button" name="" value="退出" class="jiaojuan"-->
<!--                     @click="tuichu()">-->
...
<script>
  export default{
    ...
    created() {
      ...
      this.practiceobj=JSON.parse(window.sessionStorage.getItem('practiceobj'))
       console.log(this.practiceobj)
      this.singleform=this.practiceobj.singelCheck
      this.moreform=this.practiceobj.moreCheck
      this.leftTime=this.practiceobj.time*60*1000
    ...
    methods:{
      tuichu(){
        this.$confirm('您将要退出模拟训练, 是否继续?', '提示', {
      ...
      sendAnswer(){
        ...
        this.$http.post('api/test/getStudentScore',this.addform).then(
          res=>{
      ...
      jiaojuan(){
        ...
        //       return item.id==linshi[linshiindex]
        //     })
        //     this.bjgarr[chongfuindex].value.push(this.bjgarr[i].value[0])
        //
        //     this.bjgarr.splice(i,1);
        //     i--
        //   }else{
      ...
      // getTaskInfo(){
      //   this.$http.get(`api/Test/getStudentTest/${this.queryInfo.ksid}/${this.queryInfo.loginid}`).then(res=>{
      //     console.log('获取试卷信息')
      //     console.log(res.data)
      //     if(res.data==''){
      //       console.log('已经考完了')
